...
/**
 * author：hanshaokai
 * date： 2019/3/19 0019 15:48
 * describe：
 */

public final class BitmapUtils2 {
    //--------------------------------------------------------------以下为百度人脸识别图片工具类方法
    ...
    public static Bitmap createBitmap(Context context, byte[] data, float orientation) {
        ...
        try {
            ...
            opts.inSampleSize =
                    BitmapUtils2.computeSampleSize(opts, min, BitmapUtils2.MAXLENTH * BitmapUtils2.MAXLENTH);
            opts.inJustDecodeBounds = false;
            bitmap = BitmapFactory.decodeByteArray(data, 0, data.length, opts);
            transformed = BitmapUtils2.rotateBitmap(orientation, bitmap);
            ...
            BitmapFactory.decodeByteArray(data, 0, data.length, opts);
            opts.inSampleSize =
                    BitmapUtils2.computeSampleSize(opts, -1, opts.outWidth * opts.outHeight
                            / BitmapUtils2.PIC_COMPRESS_SIZE);
            opts.inJustDecodeBounds = false;
            bitmap = BitmapFactory.decodeByteArray(data, 0, data.length, opts);
            transformed = BitmapUtils2.rotateBitmap(orientation, bitmap);
        }
    ...
    public static int computeSampleSize(BitmapFactory.Options options, int minSideLength, int maxNumOfPixels) {
        int initialSize = BitmapUtils2.computeInitialSampleSize(options, minSideLength, maxNumOfPixels);
        int roundedSize;
    ...
    public static int computeInitialSampleSize(BitmapFactory.Options options, int minSideLength,
        ...
        int upperBound =
                (minSideLength == -1) ? BitmapUtils2.IMAGEBOUND : (int) Math.min(
    ...
    public static Bitmap createBitmap(Context context, String filename, int orientatoin) {
        ...
        try {
            ...
            transformed = BitmapUtils2.rotateBitmap(orientatoin, bitmap);
        } catch (OutOfMemoryError e) {
            e.printStackTrace();
            if (bitmap != null) {
                bitmap.recycle();
