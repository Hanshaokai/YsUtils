...
import zity.net.ysutils.SportsActivity;
import zity.net.ysutils.utils.Contants;
import zity.net.ysutils.utils.LogUtils;
import zity.net.ysutils.utils.SystemUtils;
        ...
        public boolean handleMessage(Message msg) {
            ...
            if (SystemUtils.isAPPALive(getApplicationContext(), Contants.PACKAGE_NAME)) {
                ...
                Toast.makeText(getApplicationContext(), "APP被杀死，重启...", Toast.LENGTH_SHORT)
                        .show();
                LogUtils.writeLog("APP被杀死没找到进程名，job启动重启");
            }
            // 通知系统任务执行结束
            jobFinished((JobParameters) msg.obj, false);
            LogUtils.writeLog("job任务结束");
            return true;
        }
    });
